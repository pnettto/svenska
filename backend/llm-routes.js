/**
 * API routes for LLM-powered features (OpenAI and Azure Speech)
 */
...
router.post('/generate-examples', async (req, res) => {
    ...
    try {
        ...
        if (wordId) {
            try {
                const word = await dbOperations.getWordById(wordId);
                if (word) {
                    const allExamples = [...(existingExamples || []), ...examples];
                    await dbOperations.updateWord(
                        wordId,
                        word.original,
                        word.translation,
                        allExamples,
...
// Text-to-Speech endpoint using Azure Speech Service with file caching
router.post('/tts', async (req, res) => {
    const { text, wordId = null } = req.body;
